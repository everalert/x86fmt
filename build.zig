...
// TODO: add cleanup build step to `build` body
// - also add note telling user to run `./zig-clean.bat` instead if on windows
//
...
comptime {
    ...
    if (cur_zig.order(req_zig) == .eq) {
        ...
        @compileError(std.fmt.comptimePrint(error_message, .{req_zig}));
    }
}
