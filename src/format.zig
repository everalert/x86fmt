    ...
    while (fmtgen_i < lex.len) {
        line_state = switch (line_state) {
            .Label => ls: {
                ...
                Lexeme.BufAppend(out, &lex[fmtgen_i], &fmtgen_i, &fmtgen_ci);
                break :ls next_s;
            },
            .Instruction => ls: {
            ...
            .Operands => ls: {
                ...
                Lexeme.BufAppendSlice(out, lex[fmtgen_i..], &fmtgen_i, &fmtgen_ci, .{});
                break :ls .Comment;
            },
...
fn FormatGenericDirectiveLine(
    ...
    Lexeme.BufAppendOpts(out, &lex[0], &fmtgen_i, &fmtgen_ci, .{ .bHeadToLower = true });
    if (lex.len > 1) {
        PadSpaces(out, &fmtgen_ci, fmtgen_ci);
        Lexeme.BufAppendSlice(out, lex[1..], &fmtgen_i, &fmtgen_ci, .{});
    }
}
